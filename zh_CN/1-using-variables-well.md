...
当然，这也不是绝对的，代码的本质是让人可读的，以及其他的人可调试的，所以是否需要定义变量也取决于是否方便调试，例如如果定义变量`user = get_user(user_id)`，那么就能很方便的打印出`user`值是否和预期一样，而如果直接在返回值里调用，则不是很方便调试，需要在返回值外再次调用函数进行调试。
