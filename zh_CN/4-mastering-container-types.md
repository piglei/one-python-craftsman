...
列表、元组、字典、集合。这些内建容器类型的**名字**有一个共同的特点，你发现了吗？

让我来揭晓答案：*“这些名字的中文都是两个字！”*。等等！别关掉页面。让我重新说一个答案：“**这些名字都是抽象的**。”为什么这么说？因为 Python 是一门高级编程语言，**它所提供的内置容器类型，也都是经过高度封装和抽象后的结果。**和“链表”、“红黑树”、“哈希表”这些名字相比，所有 Python 内建类型的名字，都只描述了这个类型的功能特点，其他人完全没法只通过这些名字了解它们的哪怕一丁点内部细节。

拿集合来举例，你用它时，可能一点也不知道它在底层用了什么数据结构、如何分配内存。但是你只要知道*“集合存放的东西都不重复”、“集合里面可以放任意多的元素”*这些特点，就可以随意的使用它。
...
如何解释”变懒“？`range()` 内建函数是一个非常好的例子。在 Python 2 中，如果你调用 `range(100000000)`，需要等待好几秒才能拿到结果，因为它需要返回一个巨大的列表，花费了非常多的时间在内存分配与计算上。但在 Python 3 中，同样的调用马上就能拿到结果。因为函数返回的不再是列表，而是一个类型为 `range` 的懒惰对象，只有在你迭代它、或是对它进行切片时，它才会返回真正的数字给你。
...
- 尽量使用模块提供的懒惰对象：
    ...
    - 直接使用可迭代的文件对象本身，而不是 `fp.read()`、`fp.readlines()`

⭐️ **在列表头部操作多的场景使用 deque 模块**
...
字典与集合都是基于哈希表（Hash Table）数据结构实现的，所以判断某个元素是否在集合中非常快。如果你需要频繁的判断某元素是否在列表中，请考虑使用集合类型来替代列表。
...
Python 是一门“[鸭子类型](https://en.wikipedia.org/wiki/Duck_typing)”语言。所以，当我们说某个对象是什么类型时，在根本上其实可被翻译为：**这个对象提供的方法和属性，满足了该类型的特定接口规范，可以被当成这个类型完成所有操作。**而对于所有内置容器类型来说，同样如此。
